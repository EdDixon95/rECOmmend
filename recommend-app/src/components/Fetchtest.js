    ...
    return (
      <div className="col">
        ...
        {this.state.items.map((items) => (
          <div><button className="fetch-button" onClick={() => {this.searchItems({items})}}>{items.id}. {items.name}</button></div>
        ))}
        <h1>Alternatives</h1>
        {this.state.itemsAlts.map((itemsAlts) => (
          <div><h5>{itemsAlts.name}</h5>{itemsAlts.description}<br/> Stock: {itemsAlts.stock} Price: {itemsAlts.price} </div>
        ))}
      </div>
    );
  }
